...
**주입 순서는 항상 고정:** 글로벌 CLAUDE.md → 글로벌 rules 파일 → 로컬 CLAUDE.md → 자동 메모리.
...
### 2. Claude Code는 여러 모델을 사용한다

모든 작업이 Opus/Sonnet에서 실행되지 않습니다. 가벼운 작업은 저렴한 모델로 라우팅됩니다:

| 작업 | 모델 |
|------|-------|
| 시작 시 **쿼터 확인** | Haiku 3.5 |
| 메시지당 **토픽 감지** | Haiku 3.5 |
| **Bash 안전성 사전 검사** | Haiku 3.5 |
| **컨텍스트 압축** | Sonnet |
| **메인 에이전트 루프** | 사용자 선택 모델 |

매 사용자 메시지마다 숨겨진 분류 호출(`{"isNewTopic": boolean, "title": string}`)이 메인 모델 전에 실행됩니다.

### 3. MCP 도구는 토큰 절약을 위해 지연 로드된다
...
### 7. 모든 bash 명령에 안전성 검사가 실행된다

셸 명령 실행 전, **별도의 LLM 호출**이 명령 접두사를 추출하고 인젝션 패턴을 검사합니다. `` git diff $(pwd) `` 같은 명령은 `command_injection_detected`로 차단됩니다.

실행 후에는 또 다른 LLM 호출이 수정된 파일 경로를 `<filepaths>`로 추출합니다 — Claude Code가 변경 사항을 인식하는 원리입니다.
...
[⬇ v1.1.2 다운로드 (.dmg)](https://github.com/kangraemin/claude-inspector/releases/latest)

| Mac (Apple Silicon) | Mac (Intel) |
|---|---|
| `Claude-Inspector-1.1.2-arm64.dmg` | `Claude-Inspector-1.1.2-x64.dmg` |
