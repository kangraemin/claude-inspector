...
### 요청사항
- Messages 탭에서 얇은 바처럼 보이는 깨진 카드 수정

### 작업 내용
- "내가 보낸 것" 필터에서 text 없이 tool_result만 있는 user 메시지 스킵
- user 필터 시 tool_use/tool_result 블록 숨김 처리

### 변경 파일
- `public/index.html`: renderProxyMessages에서 user 필터 시 도구 관련 블록 필터링 추가

### 토큰 사용량
- 모델: claude-opus-4-6
- 이번 작업: 1,553,434 토큰 / $1.19
- 소요 시간: 7분
- 일일 누적: 94,106,858 토큰 / $44.34
...
### 요청사항
- Messages 탭 전체 섹션에 스크롤 적용

### 작업 내용
- flex 레이아웃 대신 container에 overflow:auto로 단순 스크롤 적용
- 필터/검색 바 sticky 고정, 다른 탭 전환 시 overflow 원복

### 변경 파일
- `public/index.html`: renderProxyMessages/renderProxyDetail에서 스크롤 방식 변경

### 토큰 사용량
- 모델: claude-opus-4-6
- 이번 작업: 1,214,488 토큰 / $0.96
- 소요 시간: 2분
- 일일 누적: 95,321,346 토큰 / $45.30
...
### 요청사항
- Messages 탭 스크롤 확실하게 동작하도록 수정

### 작업 내용
- container inline style을 cssText로 완전 덮어씌워 display:block + overflow-y:auto로 전환
- msgs-list CSS class 대신 inline style로 단순 flex column 렌더링
- user 필터에서 injected-only 메시지 스킵 로직 개선 (parseUserText로 typed 여부 확인)

### 변경 파일
- `public/index.html`: renderProxyMessages 스크롤 방식 및 user 필터 로직 수정

### 토큰 사용량
- 모델: claude-opus-4-6
- 이번 작업: 1,717,083 토큰 / $1.30
- 소요 시간: 2분
- 일일 누적: 97,038,429 토큰 / $46.60
---

## 22:31 (auto)

### 작업 내용
- chore: 프로젝트 CLAUDE.md 및 HANDOFF.md 추가
- UI 버그 수정 원칙과 미완료 작업 인수인계 문서 작성
...
### 요청사항
- Analysis 탭에서 Slash Command와 Skill의 관계가 불명확한 문제 개선

### 작업 내용
- Slash Command + Skill 동시 존재 시 "① 입력 → ② 실행" 흐름으로 연결 표시
- 모든 메커니즘 섹션에 설명문(analysis-desc) 추가

### 변경 파일
- `public/index.html`: renderProxyAnalysis에 흐름 연결 및 설명문 추가, .analysis-desc CSS 추가

### 토큰 사용량
- 모델: claude-opus-4-6
- 이번 작업: 3,235,550 토큰 / $2.19
- 소요 시간: 3분
- 일일 누적: 100,273,979 토큰 / $48.79
...
### 요청사항
- 검색창 한글 입력 시 자모 분리 버그 수정
- 빈 검색창에서 지우기 시 focus 풀리는 문제 수정

### 작업 내용
- compositionstart/compositionend 이벤트로 IME 조합 중 re-render 방지
- re-render 전 focus 상태를 플래그로 저장, 복원 시 참조
- 3곳 검색창(Messages, Request/Response, Analysis) 모두 적용

### 변경 파일
- `public/index.html`: IME 조합 처리 및 focus 복원 로직 추가

### 토큰 사용량
- 모델: claude-opus-4-6
- 이번 작업: 4,793,225 토큰 / $3.06
- 소요 시간: 2분
- 일일 누적: 105,067,204 토큰 / $51.85
...
### 요청사항
- README.md를 현재 기능에 맞게 대거 개편

### 작업 내용
- Simulator Mode / Proxy Mode 두 모드 구조로 재편
- 없어진 웹 모드 제거, Export/History/Proxy 상세 기능 반영
- 기술 스택 업데이트 (marked, MITM 프록시 추가)
...
### 토큰 사용량
- 모델: claude-opus-4-6
- 이번 작업: 1,028,692 토큰 / $0.81
- 소요 시간: 0분
- 일일 누적: 106,095,896 토큰 / $52.66
---

## 23:24 (auto)

### 작업 내용
- docs: README.md 오픈소스 스타일로 전면 리디자인
- - 센터 정렬 히어로 섹션, 스크린샷 2장 추가
- - 영문 중심 구성 (Proxy Mode / Simulator Mode / How It Works)
- - MIT LICENSE 파일 추가, package.json license 필드 추가

### 변경 파일
- `LICENSE`
- `README.md`
- `package.json`
- `public/screenshots/proxy-analysis.png`
- `public/screenshots/proxy-request.png`
...
### 작업 내용
- docs: 비활성화된 Simulator 모드 섹션 README에서 제거
- SHOW_SIMULATOR=false가 기본이므로 Simulator 관련 내용 삭제,
- Proxy Mode 중심으로 정리. 메커니즘 표는 How It Works에 통합.
...
### 작업 내용
- docs: What You'll Learn 섹션 추가
- Claude Code 사용자가 Inspector로 알 수 있는 5가지 인사이트 정리:
- CLAUDE.md 매 요청 주입, Slash Command→Skill 2단계 흐름,
- 시스템 프롬프트 전문 확인, 캐시 토큰 폭증 원인, Sub-Agent 격리 구조.
